...
score_W <- function(K, K0, m, D){
  ...
  #Step 3
  Pi <- cbind(R, rep(1,m))%*%solve(cbind(V,rep(1,K)))
  Pi <- pmax(Pi,0)
  temp <- rowSums(Pi)
  Pi <- apply(Pi,2,function(x) x/temp)
...
score_W = ro.globalenv['score_W']
score = ro.globalenv['score']


def generate_random_D(n, p):
    # Generate a random vocabulary of p words
    vocabulary = [f"word{i + 1}" for i in range(p)]

    # Generate random documents
    documents = []
    for i in range(n):
        Ni = np.random.randint(100, 200)  # length of document i
        document = [np.random.choice(vocabulary) for _ in range(Ni)]
        documents.append(document)

    # Initialize corpus matrix D
    D = np.zeros((p, n))

    # Compute the corpus matrix
    for i, document in enumerate(documents):
        Ni = len(document)
        word_counts = np.array(document)
        for j, word in enumerate(vocabulary):
            D[j, i] = np.count_nonzero(word_counts == word) / Ni

    return D


def generate_random_D_anchor_words(n, p, K):
    # Validate inputs
    assert K <= p, "Number of topics should be less than or equal to the vocabulary size"

    # Generate a random vocabulary of p words
    vocabulary = [f"word{i + 1}" for i in range(p)]

    # Generate unique words for each topic
    topic_unique_words = {k: vocabulary[k] for k in range(K)}

    # Distribute remaining words across topics
    remaining_words = [word for word in vocabulary if word not in topic_unique_words.values()]
    np.random.shuffle(remaining_words)
    topic_words = {k: [topic_unique_words[k]] + remaining_words[k::K] for k in range(K)}

    # Generate random documents with a topic label
    documents = []
    document_topics = []
    for i in range(n):
        topic = np.random.randint(0, K)  # select a random topic
        Ni = np.random.randint(100, 200)  # length of document i
        document = [np.random.choice(topic_words[topic]) for _ in range(Ni)]
        documents.append(document)
    ...
    return D, document_topics, topic_words


def normalize_D(D):
    n = D.shape[0]  # number of rows in D

    # Construct 1_n
    one_n = np.ones((n, 1))

    # Compute n^{-1}D1_n
    row_averages = np.dot(D, one_n) / n

    # Construct M
    M = np.diagflat(row_averages)

    # Compute M^{-1/2}
    M_inv_sqrt = np.diag(1.0 / np.sqrt(np.diag(M)))

    # Compute M^{-1/2}D
    normalized_D = np.dot(M_inv_sqrt, D)

    return normalized_D


D = normalize_D(generate_random_D(20, 20))
D_0 = normalize_D(generate_random_D_anchor_words(20, 20, 6)[0])
...
A_hat_np = np.array(A_hat_r)
W_hat_np = np.array(W_hat_r)
