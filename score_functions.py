...
import numpy as np
from preprocessing import corpus_matrix, read_docs, read_vocab
...
score_W <- function(K, K0, m, D, scatterplot=TRUE){
  library('rARPACK')
  p <- dim(D)[1]
  obj <- svds(D,K)
  Xi <- obj$v
...
def normalize_matrix(D):
    ...
    # Compute n^{-1} D 1_n
    M_diag = np.dot(D, ones_n) / n

    # Take the square root
    M_diag_sqrt_inv = np.sqrt(1 / M_diag)

    # Normalize columns of D
    D_normalized = D * M_diag_sqrt_inv[:, np.newaxis]
...
print('A_hat_np.shape:', A_hat_np.shape)
print('W_hat_np.shape:', W_hat_np.shape)
print('A_hat_np:', A_hat_np)
print('\n')
print('W_hat_np:', W_hat_np)
